...
function setFocus(taskId) {
  if (!taskId) { state.focusId=null; state.focusStartedAt=null; return; }
  const hit = findTaskById(taskId);
  if (!hit||hit.task.done||!(hit.task.text||'').trim()) { state.focusId=null; state.focusStartedAt=null; return; }
...
function reconcileFocusId() { if (state.focusId&&!getFocusTask()) state.focusId=null; }
  ...
  if (ru) {
    ...
    if (!queue.length) {
      ...
      ru.innerHTML = queue.map((item,i)=>`
        <div class="rail-upnext-item" data-set-focus="${esc(item.task.id)}">
...
async function runAIPriority() {
  ...
  try {
    ...
    output.innerHTML = '<div class="ai-priority-error">Could not reach AI — needs a local server. Run <code>npx serve .</code> in Cursor terminal, then open localhost:3000.</div>';
